...
def save_logs(output_directory, hist, y_pred, y_true, duration,
    ...
    val_acc_key = 'val_acc' if 'val_acc' in row_best_model.index else 'val_accuracy'
    lr_key = 'lr' if 'lr' in row_best_model.index else None

    df_best_model = pd.DataFrame(
        data=np.zeros((1, 6), dtype=np.float),
        index=[0],
        columns=[
    ...
    if plot_test_acc and 'val_loss' in row_best_model.index:
        df_best_model['best_model_val_loss'] = row_best_model['val_loss']

    df_best_model['best_model_train_acc'] = row_best_model[train_acc_key]

    if plot_test_acc and val_acc_key in row_best_model.index:
        df_best_model['best_model_val_acc'] = row_best_model[val_acc_key]

    if lr and lr_key is not None:
        df_best_model['best_model_learning_rate'] = row_best_model[lr_key]

    df_best_model['best_model_nb_epoch'] = index_best_model

    df_best_model.to_csv(output_directory + 'df_best_model.csv', index=False)

    if plot_test_acc and ('val_loss' in hist.history):
